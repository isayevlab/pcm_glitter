...
if __name__ == "__main__":
    ...
    EMB = sys.argv[3]
    MODEL = "XgBoost"
    embDir = "combined_reduced_embeddings.pkl"
    ...
    if EMB == "AF":
        ...
        raise ValueError(f"Invalid embedding type: {EMB}")
    ROOT = os.getcwd()
    ...
    embs = pd.read_pickle(embDir)
    # embs["embedding"] = embs["embedding"].apply(
    #     lambda x: np.mean(np.reshape(x, (-1, 1280)), axis=0))
    ...
    for i, (train_index, test_index) in enumerate(cv_splits):
        ...
        protein_train = np.array(
            [embs[embs["kinase"] == k][embCol].to_numpy()[0] for k in kinase_train])
        assert protein_train.shape[1] == embDim
        assert protein_train.shape[0] == data_train.shape[0]

        protein_test = np.array(
            [embs[embs["kinase"] == k][embCol].to_numpy()[0] for k in kinase_test])
        assert protein_test.shape[1] == embDim
    ...
    for shuffle in shuffleList:
        ...
        prot_feats = np.sum(features[:embDim])
        lig_feats = np.sum(features[embDim:])
